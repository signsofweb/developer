 ...
 */
namespace Field\Productlist\Model;
...
class Product extends \Magento\Framework\DataObject
{
     ...
     * Block cache tag
     */
     const CACHE_CATEGORY_TAG = 'field_productlist_categorytab';

    /**
     * Page cache tag
     */
    const CACHE_TAG = 'field_productlist_tab';
	/**
    ...
    public function getTopratedProducts($config = [])
    {
        ...
        ->joinField(
            ...
            'at_field_review.store_id=' . (int)$storeId,
            'field_review > 0',
            'left'
            )
        ->setPageSize(isset($config['pagesize'])?$config['pagesize']:5)
        ->setCurPage(isset($config['curpage'])?$config['curpage']:1)
        ->getSelect()->group("e.entity_id");
        $collection->getSelect()->order('field_review DESC');
        return $collection;
    }
